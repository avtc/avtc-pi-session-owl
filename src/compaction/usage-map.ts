...
// Map memkeeper's per-phase usage (PhaseUsage) into pi's `Usage` shape so the
// session_before_compact hook can return `compaction.usage` (UD32 — makes the
// bench's metric measurable for the memkeeper arms). pi's `addUsageToTotals`
...
//
// Shape notes (memkeeper → pi-ai Usage):
//  - input/output/cacheRead/cacheWrite carry straight through.
//  - `totalTokens` (pi-ai) is the per-agent context-window consumption; memkeeper's
...
//  - `cost` is flat per-phase in memkeeper (PhaseUsage.cost = cost.total); the
...
export function phaseUsageToUsage(p: PhaseUsage): Usage {
  return {
    ...
    // memkeeper doesn't track per-phase totalTokens → derive the token sum.
