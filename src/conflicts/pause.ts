...
// mid-process). Whether session-owl is actually PAUSED is decided live at each
...
// harness co-running other memory extensions) resumes session-owl mid-session.
...
/** The conflicts detected at activation (what paused session-owl + why), or null. */
export function getConflictHits(): ConflictHit[] | null {
  return conflictHits;
}

/**
 * Whether session-owl is conflict-paused RIGHT NOW: conflicts were detected at
 * activation and the operator has not opted into the deliberate last-wins
 * co-run (ignoreConflicts). Live — a settings reload flips it. A session-owl
 ...
 * applies to a session-owl that wants to run.
 */
