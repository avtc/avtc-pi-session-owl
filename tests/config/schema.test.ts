...
// flipped) so the "after init" test proves getSessionOwlSettings reads the handle.
...
// File-level teardown: clear the module handle (set by initSessionOwlSettings in the
// initialized describe) so it does NOT leak into other test files under isolate:false
// (getSessionOwlSettings would otherwise return the mock handle, not DEFAULT_CONFIG).
afterAll(() => _resetSessionOwlSettingsHandle());

describe("SESSION_OWL_SCHEMA", () => {
  it("declares every expected setting id", () => {
    const ids = SESSION_OWL_SCHEMA.settings.map((s) => s.id);
  ...
  it("every setting has a label and a defaultValue", () => {
    for (const s of SESSION_OWL_SCHEMA.settings) {
  ...
  it("every tab references only existing setting ids", () => {
    const ids = new Set(SESSION_OWL_SCHEMA.settings.map((s) => s.id));
    for (const tab of SESSION_OWL_SCHEMA.tabs) {
  ...
  it("allows null on commandResultCap and observerToolBlockCapTokens (the 'no limit' presets)", () => {
    ...
    const cap = SESSION_OWL_SCHEMA.settings.find((s) => s.id === "commandResultCap");
    expect(cap?.type).toBe("number");
    expect(hasNullPreset(cap?.presets), "commandResultCap needs a null preset").toBe(true);

    const toolCap = SESSION_OWL_SCHEMA.settings.find((s) => s.id === "observerToolBlockCapTokens");
  ...
  it("declares the root-view shape knobs' presets (null pair, custom values, six strategies)", () => {
    const target = SESSION_OWL_SCHEMA.settings.find((s) => s.id === "rootViewTargetNodes");
    ...
    const strategy = SESSION_OWL_SCHEMA.settings.find((s) => s.id === "rootViewStrategy");
  ...
  it("declares debugDumpLimit's presets (off by default, growing windows)", () => {
    const dump = SESSION_OWL_SCHEMA.settings.find((s) => s.id === "debugDumpLimit");
  ...
  it("declares `min` on the numeric knobs that need a floor", () => {
    ...
    const numeric = SESSION_OWL_SCHEMA.settings.filter((s) => s.type === "number");
...
describe("DEFAULT_CONFIG parity with schema defaults", () => {
  it("matches every schema defaultValue exactly", () => {
    for (const s of SESSION_OWL_SCHEMA.settings) {
      const key = s.id as keyof SessionOwlConfig;
      expect(DEFAULT_CONFIG[key], `DEFAULT_CONFIG.${s.id}`).toStrictEqual(s.defaultValue);
    }
  });

  it("covers every SessionOwlConfig key", () => {
    const schemaKeys = new Set(SESSION_OWL_SCHEMA.settings.map((s) => s.id));
...
describe("getSessionOwlSettings — before init", () => {
  ...
  it("returns DEFAULT_CONFIG before init (no crash for early callers)", () => {
    ...
    expect(getSessionOwlSettings()).toStrictEqual(DEFAULT_CONFIG);
  });
});
...
describe("getSessionOwlSettings — initialized", () => {
  ...
  it("initSessionOwlSettings registers the /owl:settings command with the documented options", () => {
    ...
    expect(schemaArg).toBe(SESSION_OWL_SCHEMA);
  ...
  it("returns the handle's live getSettings() after init", () => {
    const cfg = getSessionOwlSettings();
    // The mock handle yields LIVE_AFTER_INIT (enabled:false, distinct from default true).
    expect(cfg.enabled).toBe(false);
    expect(cfg.commandResultCap).toBe(25);
  });

  it("_setGetSessionOwlSettings overrides the read (the repo DI/mock pattern)", () => {
    const injected: SessionOwlConfig = { ...DEFAULT_CONFIG, enabled: false, maxBuilderPasses: 1 };
    _setGetSessionOwlSettings(() => injected);
    expect(getSessionOwlSettings()).toBe(injected);
  });

  it("_resetGetSessionOwlSettings restores the real-handle read", () => {
    const injected: SessionOwlConfig = { ...DEFAULT_CONFIG, enabled: false };
    _setGetSessionOwlSettings(() => injected);
    _resetGetSessionOwlSettings();
    // Override cleared -> falls through to the handle (set by beforeEach) -> LIVE_AFTER_INIT.
    expect(getSessionOwlSettings().enabled).toBe(false);
    expect(getSessionOwlSettings().commandResultCap).toBe(25);
  });
});
...
describe("reloadSessionOwlConfig", () => {
  ...
  it("is a no-op before initSessionOwlSettings (handle undefined — no throw)", () => {
    // handle is undefined here (cleared in beforeEach)
    expect(() => reloadSessionOwlConfig()).not.toThrow();
